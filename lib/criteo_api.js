...
class Criteo_API_Client extends API_Client {
     ...
     * @param {integer} accountId
     */
    getCampaignsByAccountId(accountId, callback){
        return this.criteoApiRequest({
            'method': 'get',
     ...
     */
    getLineItemsByCampaignId(campaignId, callback){
        return this.criteoApiRequest({
            'method': 'get',
     ...
     * @param {function} [callback] - Optional callback
     */
    getCampaignReport(query, callback){
        // TODO: do we need to figure out about date formating?
        //query.startDate = new Date(query.startDate).toISOString();
        //query.endDate = new Date(query.endDate).toISOString();
        ...
        return this.criteoApiRequest({
            'method': 'post',
            'path': `/preview/retail-media/reports/campaigns`,
            'body': JSON.stringify(payload),
            'handler': this.processJSON.bind(this),
            'callback': callback
        });
    ...
    getReportOutput(reportId, filepath, callback){
        return this.criteoApiRequest({
            ...
            'handler': this.processFile.bind(this, filepath),
            'callback': callback
        });
    }
